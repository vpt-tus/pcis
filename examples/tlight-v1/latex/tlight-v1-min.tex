...
\begin{tikzpicture} [node distance = 4cm, on grid, auto, font=\small]
    ...
    \path [-stealth, thick]
    (s1) edge [bend left] node [align=center]
        {$s(t)=0/$\\
         $s(t):=T2$} (s2)
    (s2) edge [bend left] node [align=center]
        {$s(t)=0/$\\
         $s(t):=T1$} (s3)
    (s3) edge [bend left] node[align=center]
        {$s(t)=0/$\\
         $s(t):=T2$} (s4)
    (s4) edge [bend left] node[align=center]
        {$s(t)=0/$\\
         $s(t):=T1$} (s1)
    ;
    \node[align=left] at (-2.2,-0.5) {$s(t):=T1$};
\end{tikzpicture}
\end{document}